...
- mobile onboarding screen (issue #9): manual entry OR GPS, not either/or exclusive
  - GPS resolves via expo-location's `reverseGeocodeAsync` (device OS geocoder, no network call), fields stay editable after
  - GPS submit skips backend `geocodeTown` entirely — coords already known, avoids a redundant round-trip
  - editing City/Country text after a GPS fill drops back to the manual (geocoded) path — edited text no longer guaranteed to match the GPS coords
- mobile: backend GraphQL URL hardcoded in `api/client.ts`, not an env var
  - reverses the general "envs instead of hardcoding" principle above, mobile-specific: nothing secret in a local dev URL, and this task's mobile app has no other per-environment config to justify the machinery
