...
export function OnboardingScreen({ navigation }: Props) {
  ...
  const [geocodeTown, { loading: geocodeLoading }] =
    useLazyQuery(GeocodeTownDocument);
  ...
  async function handleGetCurrentLocation() {
    ...
    try {
      ...
      if (!address?.city || !address?.country) {
        setGpsError(
          "Couldn't determine your city from your location. Enter it manually instead.",
        );
        return;
      }
  ...
  async function handleSubmit() {
    ...
    try {
      const { data } = await geocodeTown({
        variables: { town: `${city}, ${country}` },
      });
